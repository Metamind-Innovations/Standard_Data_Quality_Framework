...
def check_population_representativity(data, target_column):
    ...
    if num_classes <= 1:
        return (
            0,
            "Only one class found in data. Need at least 2 classes for meaningful representativity.",
        )
    ...
    return (
        score,
        f"Found {num_classes} classes. Ideal distribution: {ideal_percent:.1f}% each. Actual distribution: {details_str}. Score: {score:.3f}",
    )
...
def check_metadata_granularity(data, metadata=None):
    ...
    return (
        ratio,
        f"Patients with metadata: {patients_with_metadata}/{total_patients} ({ratio:.2%})",
    )
...
def check_accuracy(data, expected_ranges):
    ...
    for column, value_range in expected_ranges.items():
        if column in data.columns:
            ...
            within_range = (
                (non_na_values >= min_val) & (non_na_values <= max_val)
            ).sum()
    ...
    if error_details:
        ...
        return (
            accuracy_ratio,
            f"Checked {columns_checked} column(s). {values_within_range}/{total_values_checked} values ({accuracy_ratio:.1%}) within expected ranges. Issues: {details_str}",
        )
    else:
        return (
            accuracy_ratio,
            f"Checked {columns_checked} column(s). All {total_values_checked} values within expected ranges.",
        )
...
def check_coherence(data, column_types):
        ...
        if column in data.columns:
            if expected_type == "numeric":
                ...
                if unique_vals <= 50:
                    ...
                    inconsistent_details.append(
                        f"{column}: Expected categorical but has {unique_vals} unique values"
                    )
    ...
    if inconsistent_details:
        ...
        return (
            coherence_ratio,
            f"{consistent_features}/{total_features} features have consistent data types. Issues: {details}",
        )
    else:
        return (
            coherence_ratio,
            f"All {total_features} features have consistent data types.",
        )
...
def check_semantic_coherence(data):
    ...
    if num_duplicates == 0:
        ...
        return (
            ratio,
            f"Found {num_duplicates} duplicate column names: {details_str}. {unique_columns}/{total_columns} columns have unique names.",
        )
...
def check_completeness(data):
    ...
    if len(missing_columns) > 0:
        ...
        return (
            completeness_ratio,
            f"Data completeness: {complete_values}/{total_values} ({completeness_ratio:.1%}). Missing values by column: {details_str}",
        )
    else:
...
def check_relational_consistency(data):
    ...
    if duplicate_rows > 0:
        ...
        return (
            consistency_ratio,
            f"Found {duplicate_rows} duplicate rows out of {total_rows} ({(duplicate_rows / total_rows):.1%}). Unique rows: {unique_rows}/{total_rows}. Duplicate indices: {indices_str}",
        )
    else:
        return (
            consistency_ratio,
            f"No duplicate rows found. All {total_rows} rows are unique.",
        )
...
def check_clinical_stage_consistency(data):
    ...
    for idx, row in data.iterrows():
        if all(pd.notna(row.get(col)) for col in available_columns):
            ...
            if (
                pd.notna(t_stage)
                and pd.notna(n_stage)
                and pd.notna(m_stage)
                and pd.notna(overall)
            ):
    ...
    return (
        consistency_ratio,
        f"Clinical staging consistency: {total_checked - inconsistencies}/{total_checked} ({consistency_ratio:.1%}) cases have consistent staging. {inconsistencies} inconsistencies found.",
    )
...
def run_all_checks(data, use_case_config, metadata=None):
    ...
    results["population_representativity"] = check_population_representativity(
        data, target_column
    )
